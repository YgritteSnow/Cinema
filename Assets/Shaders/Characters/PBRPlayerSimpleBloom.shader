Shader "* Character/PBR Player Simple Bloom" {
	Properties {
	...
	SubShader {
		...
		#pragma surface surf Standard finalcolor:mycolor keepalpha vertex:vert exclude_path:prepass nolightmap noforwardadd 
		//fullforwardshadows
		...
		{
			fixed lumaSum = color.x + color.y + color.z;
			color.xyz = color.xyz / lumaSum;
			color.a = (((log(lumaSum) / 0.05826885) + 128.0) / 256.0);
		}
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			o.Metallic = mss.r;
			o.Smoothness = mss.g;
			o.Emission = mask.b * _SelfIlluminateColor + _SelfIlluminated * IN.fesnelBlink * _SelfIlluminateColor;
